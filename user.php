<?php
	...
	class User
	{
		...
		// =======================================================================
			...
			const DATABASE_LOCATION = "localhost";
		...
		// =======================================================================
			...
			const DATABASE_NAME = "shanvish_safety";
			...
			const DOMAIN_NAME = "shanvishield";
			...
			const MAILER_REPLYTO = "contact@shanvishield.com";
		...
		function __construct($params, $registration_callback=false)
		{
			...
			$this->database = new PDO('mysql:host=' .
				User::DATABASE_LOCATION .';dbname='.
				User::DATABASE_NAME, "shanvish_user",
				"Shanvi2018safety");
		...
		function process(&$registration_callback=false)
		{
			...
			if($operation != "signup" && !$this->authenticated) {
	...
	// ------------------
		...
		function register_user($username, $email, $sha1, &$registration_callback = false)
		{
			...
			if(User::unsafe_reporting) {
				$query = "SELECT username FROM Persons WHERE username LIKE '$username'";
				foreach($this->database->query($query) as $data) {
					...
					return false; }}
			else{	$query = "SELECT username FROM Persons";
				$usernames = array();
			...
			foreach($this->database->query($query) as $data) {
				...
				$this->error = "this email address is already in use by someone else.";
                                return false; }*/
                        $lastname = "";
                        if (array_key_exists("LastName", $this->params)) {
                          $lastname = $this->params["LastName"];
                        }
                        $firstname = "";
                        if (array_key_exists("FirstName", $this->params)) {
                        ...
                        $city = "";
                        if (array_key_exists("City", $this->params)) {
			...
			foreach($this->database->query($query) as $data) {
				...
				if($registration_callback !== false) { $registration_callback($username, $email, 'user'); }
				return true; }
		...
		function update_user($email, $sha1, $sha2)
                {
                  ...
                  if($this->check_password($username, $sha1)) {
                    ...
                    if($email !="") {
                      $update = "UPDATE Persons SET Email = '$email' WHERE username = '$username'";
		...
		{
			$update = "UPDATE Persons SET active = 'false' WHERE username = '$username'";
			$this->database->exec($update);
			$this->resetSession();
		...
		{
			if($username && $username !="" && $username !=User::GUEST_USER) {
				$query = "SELECT email FROM Persons WHERE username = '$username'";
		...
		{
			if($username && $username !="" && $username !=User::GUEST_USER) {
				$query = "SELECT role FROM Persons WHERE username = '$username'";
		...
		{
			$query = "SELECT token FROM Persons WHERE username = '$username'";
		...
		function update_user_token($username, $sha1)
		{
			...
			$update = "UPDATE Persons SET token = '$token' WHERE username = '$username'";
			$this->database->exec($update);

			// update the user's password
			$newpassword = $this->token_hash_password($username, $sha1, $token);
			$update = "UPDATE Persons SET password = '$newpassword' WHERE username = '$username'";
			$this->database->exec($update);
		...
		{
			$update = "UPDATE Persons SET active = 'true', last = '" . time() . "' WHERE username = '$username'";
			$this->database->exec($update);
		...
		function is_user_active($username)
		{
			$last = 0;
			$active = "false";
			$query = "SELECT last, active FROM Persons WHERE username = '$username'";
			foreach($this->database->query($query) as $data) {
